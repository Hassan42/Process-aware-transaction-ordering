...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool, publicKey string) *worker {
	worker := &worker{
		publicKey:          publicKey,
		config:             config,
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	for {
		...
		if !(strings.EqualFold(strings.ToLower(tx.From().String()[2:]), strings.ToLower(w.publicKey))) {
			fmt.Print("SKIPPING THIS TX @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
			w.current.tcount++
			txs.Shift()
			continue
		}
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	// priorityTxs, localTxs, remoteTxs := make(map[common.Address]types.Transactions), make(map[common.Address]types.Transactions), pending
	...
	// 	fmt.Println("**************************************Priority Txs **************************************")
	// 	if strings.EqualFold(strings.ToLower(account.String()[2:]), strings.ToLower(w.publicKey)) {
	// 		fmt.Println("**************************************Priority Txs **************************************")
	// 		priorityTxs[account] = txs
	// 		delete(localTxs, account)
	// 		continue
	// 	}
	...
	// 	txs := types.NewTransactionsByPriceAndNonce(w.current.signer, priorityTxs)
	// 	// fmt.Println("Committing...", txs)
